...
async function main() {
  ...
  const Movies = await hre.ethers.getContractFactory("MoviesToken");
  const movies = await Movies.deploy();
  // console.log("movies:", movies)
 await movies.deployed();

  console.log(
    `complete complie address:, address:${movies.address}`
  );

  saveFrontendFiles(movies, deployer)
}
...
//npx hardhat run scripts/deploy.js  --network rinkeby
  ...
  function saveFrontendFiles(token, deployerAccount) {
    ...
    const TokenArtifact = artifacts.readArtifactSync('MoviesToken')
...
//     const token = await Token.deploy('Hello', 'OKToken', 1, 50000)
